                    ...
                    const int deformable_group)
{
    ...
    auto weight_view = weight.view({channels_out, channels_kernel * kernel_h * kernel_w});
    auto product = at::matmul(weight_view, columns);
                                             ...
                                             int deformable_group)
{
    ...
    for (int b = 0; b < batch; b++)
    {
        ...
        auto weight_flat = weight.view({channels_out, channels_kernel * kernel_h * kernel_w});
        auto grad_output_n_flat = grad_output_n.view({channels_out, height_out * width_out});

        columns = at::matmul(weight_flat.transpose(1, 0), grad_output_n_flat);
        ...
        auto product = at::matmul(grad_output_n_flat, columns.transpose(1, 0));
        grad_weight = at::add(grad_weight, product.view({channels_out, channels_kernel, kernel_h, kernel_w}));
